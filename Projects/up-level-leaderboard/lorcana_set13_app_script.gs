...
function getAdminConfig() {
  ...
  return {
    ...
    extraStockOpen: isExtraStockOpen(),
    stock:          stock,
    r2Summary:      sheet ? computeR2Summary(sheet) : null
  };
}

// R2 sales aggregates — drives the R2 summary panel in champ-hq.
// One row = one R2 order. Customers may have multiple R2 rows.
function computeR2Summary(sheet) {
  const data = sheet.getDataRange().getValues();
  const phoneSet = {};
  let orders = 0, paid = 0, pending = 0, mismatch = 0, refunded = 0;
  let revenuePaid = 0, revenueGross = 0;
  const skuQty = {};       // skuKey -> units (active rows only)
  const recent = [];       // recent R2 orders, sorted desc by timestamp

  for (let i = 1; i < data.length; i++) {
    const row = data[i];
    if (!row[COL_NAME - 1]) continue;
    const round = String(row[COL_ORDER_ROUND - 1] || "R1");
    if (round !== "R2") continue;
    ...
    if (isRefunded) {
      ...
      revenueGross += total;
      if (status.indexOf("✅") === 0 || status.indexOf("ชำระเต็มแล้ว") >= 0 || status.indexOf("มัดจำแล้ว") >= 0) {
        paid++;
        revenuePaid += deposit;  // R2 deposit === total (full payment)
      ...
      for (let s = 0; s < SKUS.length; s++) {
        const q = parseInt(row[COL_QTY_START - 1 + s]) || 0;
        if (q > 0) skuQty[SKUS[s].key] = (skuQty[SKUS[s].key] || 0) + q;
      }
    }
    ...
    recent.push({
      ...
      status:  status,
      items:   SKUS
                 .map((s, idx) => ({ name: s.name, qty: parseInt(row[COL_QTY_START - 1 + idx]) || 0 }))
                 .filter(it => it.qty > 0)
    });
  }
  ...
  return {
    orders:        orders,
    customers:     Object.keys(phoneSet).length,
    paid:          paid,
    pending:       pending,
    mismatch:      mismatch,
    refunded:      refunded,
...
function adminSetPhase(body) {
  ...
  try {
    ...
    // Guard: closing the round while R2 is still open would leave the storefront
    // accepting R2 orders after `done` is broadcast. Force Champ to close R2 first.
    if (next === PHASE_DONE && isExtraStockOpen()) {
      throw new Error("r2_still_open_close_first");
    }
...
// Admin: update Phase 2 deadline without phase transition.
// body { deadline: "YYYY-MM-DDTHH:mm:ss+07:00", token }
function adminSetPhase2Deadline(body) {
  const dl = body && body.deadline ? String(body.deadline) : "";
  if (!dl) return { success: false, error: "no_deadline" };
  PropertiesService.getScriptProperties().setProperty("phase2_deadline_iso", dl);
  sendTelegram("📅 *Lorcana 13 — แก้ deadline Phase 2*\n→ " + dl);
  return { success: true, phase2Deadline: getPhase2Deadline() };
}
...
function doPost(e) {
  ...
  if (action === "set_phase2_deadline") {
    try {
      if (!checkAdminAuth(e)) return jsonResponse({ success: false, error: "unauthorized" });
      const body = JSON.parse(e.postData.contents);
      return jsonResponse(adminSetPhase2Deadline(body));
    } catch (err) {
      return jsonResponse({ success: false, error: String(err) });
    }
  }
  ...
  try {
    ...
    // R2 auto-close: if this submit just filled the last open R2 cap, flip the
    // R2 flag off so the storefront stops accepting new R2 orders within the
    // next poll interval (~20s).
    if (isR2) {
      try {
        const r2After = countActiveSkuTotals(sheet, "R2");
        const caps    = getSkuExtraCaps();
        let anyOpen = false;
        SKUS.forEach(s => {
          const cap = (typeof caps[s.key] === "number" && caps[s.key] > 0) ? caps[s.key] : 0;
          if (cap === 0) return;  // not for sale this round
          if ((r2After[s.key] || 0) < cap) anyOpen = true;
        });
        if (!anyOpen) {
          setExtraStockOpen(false);
          sendTelegram(
            "🔒 *Lorcana 13 — Round 2 หมดสต็อก ปิดอัตโนมัติ*\n" +
            "ทุก SKU ที่เปิดขายขายหมดแล้ว → ฟอร์มหยุดรับออเดอร์ R2 ใหม่\n" +
            "→ เปิดใหม่ได้จาก champ-hq dashboard"
          );
        }
      } catch (e) { /* swallow — auto-close best-effort */ }
    }
...
function getShippingList() {
  ...
  for (let i = 1; i < data.length; i++) {
    ...
    // R2 rows are paid full but still need shipping — include + tag with round
    const round = String(row[COL_ORDER_ROUND - 1] || "R1") || "R1";
    ...
    if (!byPhone[phone]) {
      byPhone[phone] = {
        ...
        items:        {},                  // sku.key -> qty (combined R1+R2)
        itemsR1:      {},                  // sku.key -> qty (R1 only)
        itemsR2:      {},                  // sku.key -> qty (R2 only)
        rounds:       {},                  // {R1: true, R2: true}
        total:        0,
        deposit:      0,
        remaining:    0,
        orderCount:   0,
    ...
    const perRoundBucket = round === "R2" ? rec.itemsR2 : rec.itemsR1;
    rec.rounds[round] = true;

    for (let s = 0; s < SKUS.length; s++) {
      const q = parseInt(row[COL_QTY_START - 1 + s]) || 0;
      if (q > 0) {
        rec.items[SKUS[s].key]        = (rec.items[SKUS[s].key] || 0) + q;
        perRoundBucket[SKUS[s].key]   = (perRoundBucket[SKUS[s].key] || 0) + q;
      }
    }
    ...
    rec.orderCount  += 1;
    if (status.indexOf("มัดจำแล้ว") >= 0 || status.indexOf("ชำระเต็มแล้ว") >= 0) rec.paidCount += 1;
  ...
  const customers = Object.keys(byPhone).map(p => {
    ...
    const rounds = Object.keys(rec.rounds).sort();  // ["R1"], ["R2"], or ["R1","R2"]
    return {
      phone:          rec.phone,
      name:           rec.name,
      ...
      itemsR2:        rec.itemsR2,
      rounds:         rounds,
      roundLabel:     rounds.join("+"),  // "R1", "R2", or "R1+R2"
      totalQty:       totalQty,
      total:          rec.total,
      deposit:        rec.deposit,
